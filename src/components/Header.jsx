...
import MenuSgv from '../assets/svg/MenuSvg'
    ...
    return (
        ...
        ${openNavigation ? 'bg-n-8' : 'bg-n-8/90 backdrop:blur-sm'}`}>
            ...
            <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
                ...
                <nav className={`${openNavigation ? 'flex' : 'hidden'} fixed top-[5rem] left-0 right-0 bottom-0
                 bg-n-8 lg:static lg:flex lg:mx-auto lg:bg-transparent`}>
                    ...
                    justify-center m-auto lg:flex-row'>
                        {navigation.map((item) => (
                            <a key={item.id} onClick={handleClick} href={item.url} className=
                                {`block relative font-code text-2xl uppercase text-n-1 
                ...
                <a href="#signup" className='button hidden mr-8 text-n-1/50 trasition-colors hover:text-n-1 lg:block'>
                    New Account
                </a>
                <Button className="hidden lg:flex" href="#login">Sign in</Button>

                <Button className={`ml-auto lg:hidden`} px="px-3" onClick={toggleNavigation}>
                    <MenuSgv openNavigation={openNavigation} />
                </Button>
